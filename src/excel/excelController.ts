...
import evalParams from "./parameters";
...
router.post("/csv-format", upload.single("file"), async (req, res) => {
    ...
    try {
        ...
        try {
            parameterItems = JSON.parse(parameters);
        } catch (e) {
            parameterItems = [];
        }
        console.log("Parameters: ", parameterItems);
        ...
        if (parameterItems.length > 0) {
            console.log("Parameters: ", parameterItems);
            rowsData = evalParams(rowsData, headers, parameterItems);
        }
...
const returnWorkBook = (rowsData: any) => {};
